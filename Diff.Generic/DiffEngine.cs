...
namespace Diff.Generic
{
    ...
    {
        public DiffResult<T> CreateDiffs(IEnumerable<T> oldStream, IEnumerable<T> newStream)
        {
            var pieceHash = new Dictionary<T, int>();
            var lineDiffs = new List<DiffBlock>();

            var modOld = new ModificationData<T>(oldStream.ToArray());
            var modNew = new ModificationData<T>(newStream.ToArray());
            ...
            return new DiffResult<T>(modOld.Pieces, modNew.Pieces, lineDiffs);
        }
        ...
        /// <returns></returns>
        public static EditLengthResult CalculateEditLength(int[] A, int startA, int endA, int[] B, int startB, int endB)
        {
            int N = endA - startA;
            int M = endB - startB;
            int MAX = M + N + 1;
        ...
        public static void BuildModificationData(ModificationData<T> A, ModificationData<T> B)
        {
        ...
        private static void BuildPieceHashes(IDictionary<T, int> pieceHash, ModificationData<T> data)
        {
